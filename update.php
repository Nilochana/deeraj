...
<h3 class=" tender-padding-12">Enter the tender you've added that you want to update</h3>
<br>
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
  ...
  if($_POST)
  {
        ...
        $id = $_SESSION['id1'];
        $name = $_SESSION['name'];
        $sql = "SELECT * FROM add_tender WHERE name = '$name' and id ='$id'";
        ...
        if($result)
        {
              while($row = mysqli_fetch_array($result ,MYSQL_ASSOC))
              {
                  ...
                  <a class="tender-btn" href="update1.php">Update</a>
                  </p>
                  <br>
                  </div>
                  <?php 
